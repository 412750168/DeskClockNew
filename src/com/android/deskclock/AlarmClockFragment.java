...
import com.android.deskclock.ClockFragment.TestSettingAlarm;
...
public class AlarmClockFragment extends DeskClockFragment implements
		...
		TimePickerDialog.OnTimeSetListener, View.OnTouchListener ,TestSettingAlarm{
	...
	private int mAddAlarmCount = 0;
	private int mDeleteAlarmCount = 0;
	
	private TextView textView_right_create_alarm ;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		textView_right_create_alarm = (TextView)v.findViewById(R.id.textview_right_create_alarm);
		...
		mAddAlarmButton = (Button) v.findViewById(R.id.alarm_add_alarm);
		mAddAlarmButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(mAddAlarmCount == 0){
					...
					if(mAlarmsList.getAdapter().getCount()== 10){
						Toast.makeText(getActivity(), R.string.i_alarms_number, Toast.LENGTH_LONG).show();
					}else{
		...
		mAdapter.registerDataSetObserver(new DataSetObserver() {
			...
			public void onChanged() {
				...
				if (count == 0) {
					...
					textView_right_create_alarm.setVisibility(View.VISIBLE);
	...
	public void onTimeSet(RadialPickerLayout view, int hourOfDay, int minute) {
		if (mSelectedAlarm == null) {
			...
			a.enabled = true;
			asyncAddAlarm(a);
			Log.d("***zzl***********this is alarm format 12,24:::"+hourOfDay+"::"+minute);
		} else {
	...
	public void updateAlarm(Alarm alarm, int hourOfDay,int minute,int days[]){
		...
		for(int i=0;i<7 ; i++)
			if(days[i] != 0)
				alarm.daysOfWeek.setDaysOfWeek(true, days[i] );
		asyncUpdateAlarm(alarm, true);
		for(int j=0;j<7 ;j++)
			if(days[j] == 0){
				alarm.daysOfWeek.setDaysOfWeek(false, j+1);				
			}
	...
	public void updateAlarm(Alarm alarm, int hourOfDay,int minute,int days[],Uri uri,String label){
	...
	public void setAddNewAlarm(int hourOfDay,int minute){
		Alarm a = new Alarm();
		a.alert = RingtoneManager.getActualDefaultRingtoneUri(
				getActivity(), RingtoneManager.TYPE_ALARM);
		if (a.alert == null) {
		...
		a.minutes = minute;
		a.enabled = true;
		//a.daysOfWeek.setDaysOfWeek(true, 3);// this is test zzl
		asyncAddAlarm(a);
	}
	
	
	public void setAddNewAlarm(int hourOfDay,int minute,int days[]){
		Alarm a = new Alarm();
		a.alert = RingtoneManager.getActualDefaultRingtoneUri(
				getActivity(), RingtoneManager.TYPE_ALARM);
		if (a.alert == null) {
	...
	/*	File file = new File("/storage/external_storage/sdcard1/1.mp3");
		...
		*/
		...
		for(int i=0;i<7 ; i++)
			if(days[i] != 0)
				a.daysOfWeek.setDaysOfWeek(true, days[i] );
		//a.daysOfWeek.setDaysOfWeek(true, 3);// this is test zzl
		asyncAddAlarm(a);
	}
	
	public void setAddNewAlarm(int hourOfDay,int minute,int days[],Uri uri,String label){
		Alarm a = new Alarm();
		a.alert = uri;
		a.hour = hourOfDay;
		a.minutes = minute;
	    ...
	    textView_right_create_alarm.setVisibility(View.GONE);
	...
	public class AlarmItemAdapter extends CursorAdapter {
		...
		public class ItemHolder {
			...
			//zzl
			 ...
			 TextView textView_alarm_repeat_days;
							...
							if (firstAnimation) {
								...
								child.animate().withEndAction(new Runnable() {
									...
									public void run() {
										...
										// If there was an added view, animate
										// it in after
		...
		private void setNewHolder(View view) {
			...
			holder.clock = (TextClockDiff) view.findViewById(R.id.digital_clock);
			...
			holder.b_sun = (ToggleButton)view.findViewById(R.id.togglebutton_alarm_sun);
			holder.b_mon = (ToggleButton)view.findViewById(R.id.togglebutton_alarm_mon);
			holder.b_tue = (ToggleButton)view.findViewById(R.id.togglebutton_alarm_tue);
			holder.b_wed = (ToggleButton)view.findViewById(R.id.togglebutton_alarm_wed);
			holder.b_thr = (ToggleButton)view.findViewById(R.id.togglebutton_alarm_thr);
			holder.b_fri = (ToggleButton)view.findViewById(R.id.togglebutton_alarm_fri);
			holder.b_sat = (ToggleButton)view.findViewById(R.id.togglebutton_alarm_sat);
			
			holder.textView_alarm_repeat_days = (TextView)view.findViewById(R.id.textview_alarm_repeate_days);
			view.setTag(holder);
		}
		...
		public void bindView(final View view, Context context,
			...
			itemHolder.clock.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View view) {
					...
					*/
					//
					/*AlarmDialogDelete alarmDialogDelete = new AlarmDialogDelete(getActivity(), alarm, view);
					alarmDialogDelete.setAlarmClockFragment(AlarmClockFragment.this);
					alarmDialogDelete.showDialog();*/
					AlarmDialog alarmDialog = new AlarmDialog(getActivity(), alarm, view);
					alarmDialog.setAlarmClockFragment(AlarmClockFragment.this);
					alarmDialog.dialogShow();
					//////
		...
		*/
			...
			itemHolder.b_sun.setChecked(false);				
			itemHolder.b_mon.setChecked(false);				
			itemHolder.b_tue.setChecked(false);					
			itemHolder.b_wed.setChecked(false);					
			itemHolder.b_thr.setChecked(false);				
			itemHolder.b_fri.setChecked(false);					
			itemHolder.b_sat.setChecked(false);
			...
			if(setDays.size() >0){
				for (int i = 0; i < 7; i++) {
					if (setDays.contains(DAY_ORDER[i])) {
						switch(i){
						case 0:
							itemHolder.b_sun.setChecked(true);
							stringBuilder.append(mContext.getResources().getString(R.string.dialog_sun)+" ");
							break;
						case 1:
							itemHolder.b_mon.setChecked(true);
							stringBuilder.append(mContext.getResources().getString(R.string.dialog_mon)+" ");
							break;
						case 2:
							itemHolder.b_tue.setChecked(true);
							stringBuilder.append(mContext.getResources().getString(R.string.dialog_tue)+" ");
							break;
						case 3:
							itemHolder.b_wed.setChecked(true);
							stringBuilder.append(mContext.getResources().getString(R.string.dialog_wed)+" ");

							break;
						case 4:
							itemHolder.b_thr.setChecked(true);
							stringBuilder.append(mContext.getResources().getString(R.string.dialog_thr)+" ");

							break;
						case 5:
							itemHolder.b_fri.setChecked(true);
							stringBuilder.append(mContext.getResources().getString(R.string.dialog_fri)+" ");

							break;
						case 6:
							itemHolder.b_sat.setChecked(true);
							stringBuilder.append(mContext.getResources().getString(R.string.dialog_sat)+" ");
				...
				stringBuilder.append(mContext.getResources().getString(R.string.dialog_one_day)+" ");
			}
			
			itemHolder.textView_alarm_repeat_days.setText(stringBuilder.toString());
			
			itemHolder.b_sun.setEnabled(false);				
			itemHolder.b_mon.setEnabled(false);				
			itemHolder.b_tue.setEnabled(false);					
			itemHolder.b_wed.setEnabled(false);					
			itemHolder.b_thr.setEnabled(false);				
			itemHolder.b_fri.setEnabled(false);					
			itemHolder.b_sat.setEnabled(false);
		}
	...
	private void startCreatingAlarm() {
		...
		/*AlarmUtils.showTimeEditDialog(getChildFragmentManager(), null,
				...
				DateFormat.is24HourFormat(getActivity()));
	    */// zzl
	/*	AlarmDialogNew alarmDialogNew = new AlarmDialogNew(getActivity());
		alarmDialogNew.setAlarmClockFragment(this);
		alarmDialogNew.showDialog();
		*/
		AlarmDialog alarmDialog = new AlarmDialog(getActivity());
		alarmDialog.setAlarmClockFragment(this);
		alarmDialog.dialogShow();
	}
		...
		final AsyncTask<Void, Void, AlarmInstance> updateTask = new AsyncTask<Void, Void, AlarmInstance>() {
			...
			protected void onPostExecute(AlarmInstance instance) {
				if (popToast && instance != null) {
					AlarmUtils.popAlarmSetToast(context, instance
							.getAlarmTime().getTimeInMillis());
					if(mAlarmsList.getChildCount() > 0)
						textView_right_create_alarm.setVisibility(View.GONE);
					else textView_right_create_alarm.setVisibility(View.VISIBLE);
				}
			}
		};
		updateTask.execute();
	...
	@Override
	public void setTestSettingAlarm() {
		// TODO Auto-generated method stub
